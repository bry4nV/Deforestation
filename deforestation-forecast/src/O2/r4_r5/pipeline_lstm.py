...
def preparar_X_lstm(X_tensor):
    if X_tensor.ndim == 2:
        X_tensor = X_tensor.unsqueeze(-1)

    return X_tensor.float()
...
def evaluar(model, X_tensor, y_tensor):
    ...
    X = preparar_X_lstm(X_tensor).to(DEVICE)
...
# ============================================================
# FASE 1: Grid search exploratorio LSTM
# ============================================================
...
def pipeline_lstm(
    ...
    print("\n[INFO] Pipeline LSTM — búsqueda exploratoria de hiperparámetros")
    print("=" * 70)
    ...
    for idx, ((w, data), hidden_size, num_layers, dropout, epochs, lr, batch_size) in enumerate(grid, start=1):
        ...
        nombre = (
            f"LSTM_w{w}_h{hidden_size}_l{num_layers}"
...
# ============================================================
# FASE 2: Entrenamiento final LSTM
# ============================================================
...
def entrenar_config_final_lstm(
    ...
    hidden_size = int(final_config["hidden_size"])
    num_layers = int(final_config["num_layers"])
    ...
    nombre = (
        f"LSTM_FINAL_w{window_size}_h{hidden_size}_l{num_layers}"
        f"_d{dropout}_e{epochs}_lr{lr}_b{batch_size}"
    )
