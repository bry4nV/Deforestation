...
def main():
    ...
    # ========================================================================
    # FASE 3: SCORE DE POTENCIAL HISTÓRICO
    # ========================================================================
    print("\n" + "="*70)
    print("FASE 3: SCORE DE POTENCIAL HISTÓRICO")
    print("="*70 + "\n")
    
    pesos = {
        'w_frecuencia': PARAMS_ZONIFICACION['w_frecuencia'],
        'w_recencia': PARAMS_ZONIFICACION['w_recencia'],
        'w_persistencia': PARAMS_ZONIFICACION['w_persistencia'],
        'w_exposicion': PARAMS_ZONIFICACION['w_exposicion'],
        'w_densidad': PARAMS_ZONIFICACION['w_densidad']
    }
    
    print(f"[INFO] Pesos: {pesos}")
    
    score = calcular_score(features, pesos)
    estadisticas_score(score)
    
    # Suavizar score
    print("[INFO] Suavizando score espacialmente...")
    score_suavizado = suavizar_score(score, sigma=PARAMS_ZONIFICACION['sigma_suavizado'])
    
    # Guardar
    guardar_score(score, transform, crs, os.path.join(FEATURES_DIR, "score_raw.tif"))
    guardar_score(score_suavizado, transform, crs, os.path.join(FEATURES_DIR, "score_suavizado.tif"))
    
    # ========================================================================
    # FASE 4: IDENTIFICACIÓN DE NÚCLEOS
    # ========================================================================
    nucleos_labels, nucleos_info = identificar_nucleos(score_suavizado, transform, PARAMS_ZONIFICACION)
    
    guardar_nucleos(nucleos_labels, transform, crs, os.path.join(NUCLEOS_DIR, "nucleos.tif"))
    
    # ========================================================================
    # FASE 5: EXPANSIÓN A ZONAS CANDIDATAS
    # ========================================================================
    bosque_remanente = calcular_bosque_remanente_actual(bosque_stack)
    
    zonas_candidatas = expandir_nucleos_sobre_bosque(
        nucleos_labels, 
        bosque_remanente, 
        transform, 
        PARAMS_ZONIFICACION
    ...
    estadisticas_expansion(nucleos_labels, zonas_candidatas, transform)
    guardar_zonas(zonas_candidatas, transform, crs, os.path.join(ZONAS_DIR, "zonas_candidatas.tif"))
    
    # ========================================================================
    # FASE 6: REGULARIZACIÓN
    # ========================================================================
    zonas_regularizadas, metricas, flags = regularizar_zonas(
        zonas_candidatas,
        nucleos_labels,
        perdida_stack,
        bosque_stack,
    ...
    guardar_zonas_regularizadas(zonas_regularizadas, transform, crs, os.path.join(ZONAS_DIR, "zonas_regularizadas.tif"))
    
    # ========================================================================
    # FASE 7: SELECCIÓN DE ZONAS FINALES
    # ========================================================================
    zonas_finales, stats_finales = seleccionar_zonas_finales(
        zonas_regularizadas,
        perdida_stack,
        bosque_stack,
        transform,
    ...
    guardar_zonas_finales(zonas_finales, transform, crs, os.path.join(ZONAS_DIR, "zonas_finales.tif"))
    exportar_estadisticas_csv(stats_finales, os.path.join(ZONAS_DIR, "estadisticas_zonas.csv"))
    
    # ========================================================================
    # FASE 11: CONSTRUCCIÓN DEL PANEL ZONA-AÑO
    # ========================================================================
    panel = construir_panel_zona_anio(
        zonas_finales,
        bosque_stack,
        perdida_stack,
        ANIOS,
    ...
    panel = agregar_flags_panel(panel, PARAMS_ZONIFICACION)
    
    # Exportar
    exportar_panel(panel, os.path.join(PANEL_DIR, "panel_zona_anio.csv"))
    generar_estadisticas_panel(panel, os.path.join(PANEL_DIR, "reporte_panel.txt"))
