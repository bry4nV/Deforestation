...
def construir_rios_lagos(distritos_gdf):
    """Calcula la fraccion de rios y lagos (clase 33) por distrito y anio (1985-2024).
    ...
    """
    if os.path.exists(RIOS_LAGOS_CSV) and os.path.exists(RIOS_LAGOS_META):
        logger.info("[SKIP] Rios y lagos ya calculados — cargando CSV existente")
        return pd.read_csv(RIOS_LAGOS_CSV)

    n_distritos = len(distritos_gdf)
    logger.info(f"Calculando rios_lagos 1985-2024 para {n_distritos} distritos...")
    ...
    for anio in ANIOS:
        ...
        for stat, (_, row) in zip(stats, distritos_gdf.iterrows()):
            ...
            pixeles_clase   = sum(conteos.get(c, 0) for c in CLASES_RIOS_LAGOS)
    ...
    guardar_csv(df, RIOS_LAGOS_CSV)
    guardar_metadatos(
        {
            "variable":            "rios_lagos",
            "fuente":              MAPBIOMAS_AMAZONIA_PATRON,
            "clases_mapbiomas":    str(sorted(CLASES_RIOS_LAGOS)),
    ...
    logger.info(f"[OK] Rios y lagos calculados: {len(df)} registros")
    return df
